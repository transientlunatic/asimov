...
def get_psds(rundir):
    ...
    files = []
    for det in dets:
        psd_dir = "ROQdata/0/BayesWave_PSD_{det}/post/clean/glitch_median_PSD_forLI_{det}.dat"
        psd_file = os.path.join(rundir, psd_dir)
        exists = os.path.exists(psd_file)
        if exists:
            files.append(psd_files)
    return files
            ...
            if "C01::Robot" in issue.labels:
                ...
                if cluster_id:
                    ...
                    if not 'JobStatus' in job:
                        ...
                        issue.notes.create({'body': '# Run problem\nThe Prod0 job ran into an unknown error at {}'.format(datetime.datetime.now())})
                ...
                if cluster_id:
                    ...
                    if not 'JobStatus' in job:
                        ...
                        issue.notes.create({'body': '# Run problem\nThe Prod1 job ran into an unknown error at {}'.format(datetime.datetime.now())})
                    elif job['JobStatus'] == 2:
                        # The job is still running
                        pass
                    elif job['JobStatus'] == 4:
