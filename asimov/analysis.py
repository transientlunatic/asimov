...
class Analysis:
    ...
    def results(self, filename=None, handle=False, hash=None):
        ...
        if not filename:
            ...
            return open(store.fetch_file(self.subject.name, self.name, filename, hash), "r")
        else:
...
class GravitationalWaveTransient(SimpleAnalysis):
    ...
    def __init__(self,
                 ...
                 comment=None,
                 **kwargs):
        super().init(subject, name, pipeline, status=None, comment=None,
                     **kwargs)
        self._checks()
    ...
    def _checks(self):
        ...
        if self.quality:
            if ("high-frequency" not in self.quality) and ("sample-rate" in self.quality):
                ...
                if self.meta['quality']['high-frequency'] != int(0.875 * self.meta['quality']['sample-rate']/2):
                    warn("The upper-cutoff frequency is not equal to 0.875 times the Nyquist frequency.")
